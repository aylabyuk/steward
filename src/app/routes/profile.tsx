...
const RAIL_ELSEWHERE = [{ to: "/settings", label: "Ward settings" }];

function nowLabel(): string {
  return new Date().toLocaleTimeString([], { hour: "numeric", minute: "2-digit" });
}

/** Profile page — identity, notifications, and session in one
 *  surface with an explicit Save/Discard savebar. Device toggles
 *  and sign-out are imperative; the savebar tracks displayName +
 *  notificationPrefs. */
export function ProfilePage(): React.ReactElement {
  ...
  const source = useMemo<Draft | null>(() => {
    ...
    return {
      ...
      prefs: withDefaults(me.data.notificationPrefs),
    };
  }, [me]);
  ...
  const dirty =
    ...
    JSON.stringify(draft.prefs) !== JSON.stringify(source.prefs);
    ...
    <main className="pb-24">
      ...
      <header className="mb-6">
        ...
        <p className="font-serif italic text-[16px] text-walnut-2 mt-1">
          Your name, picture, and how Steward reaches you.
        </p>
      </header>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-[1fr_180px] gap-8 items-start">
        <div>
          ...
          <NotificationsSection
            ...
            prefs={draft.prefs}
            onPrefsChange={(prefs) => setDraft({ ...draft, prefs })}
          />
          <SessionSection />
        </div>
